...
class IPlanarStrategy
{
  ...
  virtual kai::ops::VLType get_vl_type(void) const = 0;
...
struct PlanarKernelType<TInput, TWeight, TOutput, int32_t, kai::ops::Requantize32>
{
  typedef void (*Type)(
    const TInput *, size_t ld_in_row, size_t ld_in_col, size_t ld_in_vl,
    unsigned int pad_top, unsigned int valid_input_rows,
  ...
  static inline void execute(
    ...
    const kai::ops::Requantize32 &qp, const WorkspaceType *
  )
  {
    fn(
      inptr, ld_in_row, ld_in_col, ld_in_vl,
...
class PlanarStrategy : public IPlanarStrategy<OutputStage>
{
  ...
  kai::ops::VLType get_vl_type(void) const override { return m_vl_type; }

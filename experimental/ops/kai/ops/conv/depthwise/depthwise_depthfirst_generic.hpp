...
{
  using KernelType = std::function<void(const TInput *const *const, TOutput *const *const, const void *, const kai::ops::Requantize32 &, unsigned int, unsigned int)>;
};
...
class GenericDepthfirstKernelStrategy
{
  ...
  kai::ops::VLType m_vl_type;
  unsigned int m_accumulator_depth_vl;

  public:
  GenericDepthfirstKernelStrategy(unsigned int n_output_points, kai::ops::VLType vl_type, unsigned int accumulator_depth_vl=1)
  ...
  virtual kai::ops::VLType get_vl_type() const { return m_vl_type; }
...
class GenericDepthfirstStrategy : public DepthwiseDepthfirstStrategyCommon<TInput, TWeight, TOutput, TAccum, OutputStage>
{
  ...
  kai::ops::VLType get_vl_type(void) const override { return m_strategy->get_vl_type(); }
...
template <>
struct DepthwiseDepthfirstGenericKernelCall<kai::ops::Requantize32>
{
  template <typename StratType, typename WorkspaceType>
  static void execute(
    const StratType *strat, const WorkspaceType *ws, const kai::ops::Requantize32 &qp,

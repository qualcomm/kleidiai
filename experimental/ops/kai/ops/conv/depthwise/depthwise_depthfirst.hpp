...
: public DepthwiseDepthfirstStrategyCommon<TInput, TWeight, TOutput, int32_t, kai::ops::Requantize32>
{
  ...
  void pack_parameters(
    ...
    const void *biases, const kai::ops::Requantize32 &,
  ...
  ) const
  {
    ...
    if (this->uses_intermediate_array()) {
      ...
      multiplied_input.load_from(input.base, input.ld_row, input.ld_col,
                                 ...
                                 input_i, input_j, output_channel_start, args.channel_multiplier);
...
template <>
struct WorkspaceFinalElement<int32_t, kai::ops::Requantize32, false>
{
...
template <typename TInput, typename TWeight, typename TOutput, typename TAccum>
struct Invoke<TInput, TWeight, TOutput, TAccum, kai::ops::Requantize32>
{
  constexpr static bool supports_direct_kernel = false;

  template <typename Strat, typename Workspace>
  static inline void indirect(const Strat *strat, const Workspace *ws, const kai::ops::Requantize32 &qp, const void *params, const TAccum *, unsigned int n_channels)
  {
    strat->get_kernel()(
  ...
  static inline void direct(
    const Strat *, const Workspace *, const kai::ops::Requantize32 &,
...
template <>
inline void stash_bias(kai::ops::Requantize32 &qp, const void *bias) __attribute__ ((unused));

template <>
inline void stash_bias(kai::ops::Requantize32 &qp, const void *bias)
{
    ...
    if (Invoker::supports_direct_kernel)
    {
      ...
      if (this->uses_intermediate_array()) {
        ...
        multiplied_input.load_from(input.base,
          input.ld_row, input.ld_col,
          args.input_rows, args.input_cols,
          start_input_i, start_input_j, output_channel_start, args.channel_multiplier);
      ...
      for (unsigned int tile_i = 0; tile_i < n_tile_rows; tile_i++)
      {
        ...
        for (unsigned int tile_j = 0; tile_j < n_tile_cols; tile_j++)
        {
          ...
          if (this->uses_intermediate_array()) {
            ...
            multiplied_input.load_from(input.base,
              input.ld_row, input.ld_col,
              args.input_rows, args.input_cols, input_i, input_j, output_channel_start, args.channel_multiplier);
          } else {

...
class ActivationsElement
{
  ...
  static void *initialise(WorkspaceType *ws, void *buffer, const WorkspaceArgs<StratType, OutputStage> &args)
  {
    ...
    {
      case kai::ops::Activation::Type::BoundedReLU:
        ws->activation_max = static_cast<T>(args.depthwise_args.activation.param1);
        // Fall through
      case kai::ops::Activation::Type::ReLU:
...
/* Activation clamps are contained within `kai::ops::Requantize32`, so if the
 * output stage is one of these we substitute in an empty workspace element.
 */
template <typename T>
class ActivationsElement<T, kai::ops::Requantize32> : public EmptyElement
{
};
 ...
 */
template <> char get_input_buffer_fill_value(const kai::ops::Requantize32 &qp) __attribute__ ((unused));
template <> char get_input_buffer_fill_value(const kai::ops::Requantize32 &qp)
{
  return qp.a_offset;
}
...
class RequantizationParametersElement
{
  ...
  template <typename StratType>
  static size_t get_element_size(const WorkspaceArgs<StratType, kai::ops::Requantize32> &args)
  {
  ...
  static void *initialise(WorkspaceType *ws, void *buffer, const WorkspaceArgs<StratType, kai::ops::Requantize32> &args)
  {
  ...
  protected:
  template <typename StratType>
  static size_t sizeof_bias(const WorkspaceArgs<StratType, kai::ops::Requantize32> &args)
  {
  ...
  template <typename StratType>
  static size_t sizeof_requant_muls(const WorkspaceArgs<StratType, kai::ops::Requantize32> &args)
  {
  ...
  template <typename StratType>
  static size_t sizeof_requant_shifts(const WorkspaceArgs<StratType, kai::ops::Requantize32> &args)
  {

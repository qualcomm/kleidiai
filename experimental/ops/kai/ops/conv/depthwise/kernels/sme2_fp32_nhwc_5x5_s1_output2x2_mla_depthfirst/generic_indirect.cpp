//
// SPDX-FileCopyrightText: Copyright 2021, 2025-2026 Arm Limited and/or its affiliates <open-source-office@arm.com>
//
// SPDX-License-Identifier: Apache-2.0
//

// clang-format off


#include <cstddef>
#include <cstdint>

#if defined(__aarch64__)

namespace kai {
namespace ops {
namespace depthwise {

void sme2_fp32_nhwc_5x5_s1_output2x2_mla_depthfirst_indirect_impl(
  const float *const *const input_ptrs,
  float *const *const outptrs,
  const void *params,
  unsigned int n_channels,
  const float activation_min,
  const float activation_max
)
{
  struct Args
  {
    float *const *outptrs;
    const void *params;
    const float min, max;
    const float *inptrs[36];

    Args(
      const float *const *const input_ptrs,
      float *const *const outptrs,
      const void *const params,
      const float min,
      const float max
    ) : outptrs(outptrs), params(params), min(min), max(max)
    {
      inptrs[0] = input_ptrs[0];
      inptrs[1] = input_ptrs[1];
      inptrs[2] = input_ptrs[6];
      inptrs[3] = input_ptrs[7];
      inptrs[4] = input_ptrs[2];
      inptrs[5] = input_ptrs[8];
      inptrs[6] = input_ptrs[3];
      inptrs[7] = input_ptrs[4];
      inptrs[8] = input_ptrs[11];
      inptrs[9] = input_ptrs[12];
      inptrs[10] = input_ptrs[9];
      inptrs[11] = input_ptrs[10];
      inptrs[12] = input_ptrs[5];
      inptrs[13] = input_ptrs[13];
      inptrs[14] = input_ptrs[14];
      inptrs[15] = input_ptrs[15];
      inptrs[16] = input_ptrs[16];
      inptrs[17] = input_ptrs[17];
      inptrs[18] = input_ptrs[18];
      inptrs[19] = input_ptrs[19];
      inptrs[20] = input_ptrs[20];
      inptrs[21] = input_ptrs[21];
      inptrs[22] = input_ptrs[22];
      inptrs[23] = input_ptrs[23];
      inptrs[24] = input_ptrs[24];
      inptrs[25] = input_ptrs[25];
      inptrs[26] = input_ptrs[26];
      inptrs[27] = input_ptrs[27];
      inptrs[28] = input_ptrs[28];
      inptrs[29] = input_ptrs[29];
      inptrs[30] = input_ptrs[30];
      inptrs[31] = input_ptrs[31];
      inptrs[32] = input_ptrs[32];
      inptrs[33] = input_ptrs[33];
      inptrs[34] = input_ptrs[34];
      inptrs[35] = input_ptrs[35];

    }
  };

  Args params_struct(input_ptrs, outptrs, params,
                     activation_min, activation_max);

  __asm__ __volatile__(
    "ldr x20, [%x[params_struct], %[offsetof_args_outptrs]]\n"
    "add x17, %x[params_struct], %[offsetof_Args_inptrs]\n"
    ".inst 0xd503477f  // SMSTART ZA\n"
    "mov x16, #0\n"
    "ldr x15, [%x[params_struct], %[offsetof_args_params]]\n"
    ".inst 0x25207810  // ptrue pn8.b\n"
    "whilelt p3.s, XZR, %x[n_channels]\n"
    "ptrue p2.b\n"
    "cntw x14\n"
    "ldp x13, x12, [x20, #0]\n"
    "cmp x14, %x[n_channels]\n"
    "ld1rw { z18.s }, p2/Z, [%x[params_struct], %[offsetof_args_min]]\n"
    "sub x11, XZR, x14\n"
    "ldp x10, x9, [x20, #0x10]\n"
    "ld1w { z17.s }, p2/Z, [x15]\n"
    "addvl x15, x15, #1\n"
    "ldp x21, x20, [x17, #0]\n"
    ".inst 0xa040c1e0  // ld1w { z0.s-z3.s }, pn8.b/Z, [x15]\n"
    "addvl x15, x15, #4\n"
    "ldp x27, x26, [x17, #0x10]\n"
    "ld1rw { z16.s }, p2/Z, [%x[params_struct], %[offsetof_args_max]]\n"
    "ldp x25, x24, [x17, #0x20]\n"
    "ld1w { z4.s }, p2/Z, [x15]\n"
    "addvl x15, x15, #1\n"
    "ld1w { z5.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ldp x23, x22, [x17, #0x30]\n"
    "ld1w { z6.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ldp x21, x20, [x17, #0x40]\n"
    "ld1w { z7.s }, p3/Z, [x27, x16, LSL #2]\n"
    "ld1w { z8.s }, p3/Z, [x26, x16, LSL #2]\n"
    "ld1w { z9.s }, p3/Z, [x25, x16, LSL #2]\n"
    "ld1w { z13.s }, p3/Z, [x24, x16, LSL #2]\n"
    "ld1w { z11.s }, p3/Z, [x23, x16, LSL #2]\n"
    "ld1w { z12.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ld1w { z10.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ld1w { z14.s }, p3/Z, [x20, x16, LSL #2]\n"
    "bge 2f\n"
    "1:"  // Channel loop
    "movprfx z28, z17\n fmla z28.s, p2/M, z0.s, z5.s\n"
    "movprfx z29, z17\n fmla z29.s, p2/M, z0.s, z6.s\n"
    "ldr x21, [x17, #0x50]\n"
    "whilelt p1.s, x14, %x[n_channels]\n"
    "movprfx z30, z17\n fmla z30.s, p2/M, z0.s, z7.s\n"
    "movprfx z31, z17\n fmla z31.s, p2/M, z0.s, z8.s\n"
    "ldr x20, [x17, #0x58]\n"
    "ld1w { z0.s }, p2/Z, [x15]\n"
    "ldr x22, [x17, #0x60]\n"
    "incw x11\n"
    "mov p0.b, p3.b\n"
    "ld1w { z5.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ldr x25, [x17, #0x68]\n"
    "fmla z28.s, p2/M, z1.s, z6.s\n"
    "fmla z29.s, p2/M, z1.s, z9.s\n"
    "ld1w { z6.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ldr x24, [x17, #0x70]\n"
    "fmla z30.s, p2/M, z1.s, z8.s\n"
    "fmla z31.s, p2/M, z1.s, z13.s\n"
    "ld1w { z1.s }, p2/Z, [x15, #1, MUL VL]\n"
    "ldr x21, [x17, #0x78]\n"
    "ldr x27, [x17, #0x80]\n"
    "ldr x20, [x17, #0x88]\n"
    "fmla z28.s, p2/M, z2.s, z9.s\n"
    "fmla z29.s, p2/M, z2.s, z11.s\n"
    "ld1w { z9.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ldr x23, [x17, #0x90]\n"
    "fmla z30.s, p2/M, z2.s, z13.s\n"
    "fmla z31.s, p2/M, z2.s, z5.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #2, MUL VL]\n"
    "ldr x26, [x17, #0x98]\n"
    "ldr x22, [x17, #0xa0]\n"
    "fmla z28.s, p2/M, z3.s, z11.s\n"
    "fmla z29.s, p2/M, z3.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x25, x16, LSL #2]\n"
    "ldr x25, [x17, #0xa8]\n"
    "fmla z30.s, p2/M, z3.s, z5.s\n"
    "fmla z31.s, p2/M, z3.s, z6.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #3, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z12.s\n"
    "fmla z29.s, p2/M, z4.s, z9.s\n"
    "ld1w { z12.s }, p3/Z, [x24, x16, LSL #2]\n"
    "ldr x24, [x17, #0xb0]\n"
    "fmla z30.s, p2/M, z4.s, z6.s\n"
    "fmla z31.s, p2/M, z4.s, z10.s\n"
    "ld1w { z9.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ldr x21, [x17, #0xb8]\n"
    "ld1w { z4.s }, p2/Z, [x15, #4, MUL VL]\n"
    "fmla z28.s, p2/M, z0.s, z7.s\n"
    "fmla z29.s, p2/M, z0.s, z8.s\n"
    "fmla z30.s, p2/M, z0.s, z14.s\n"
    "fmla z31.s, p2/M, z0.s, z11.s\n"
    "ld1w { z0.s }, p2/Z, [x15, #5, MUL VL]\n"
    "fmla z28.s, p2/M, z1.s, z8.s\n"
    "fmla z29.s, p2/M, z1.s, z13.s\n"
    "ld1w { z8.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ldr x28, [x17, #0xc8]\n"
    "fmla z30.s, p2/M, z1.s, z11.s\n"
    "fmla z31.s, p2/M, z1.s, z12.s\n"
    "ld1w { z1.s }, p2/Z, [x15, #6, MUL VL]\n"
    "fmla z28.s, p2/M, z2.s, z13.s\n"
    "fmla z29.s, p2/M, z2.s, z5.s\n"
    "ld1w { z13.s }, p3/Z, [x27, x16, LSL #2]\n"
    "ldr x20, [x17, #0xc0]\n"
    "fmla z30.s, p2/M, z2.s, z12.s\n"
    "fmla z31.s, p2/M, z2.s, z9.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #7, MUL VL]\n"
    "addvl x15, x15, #16\n"
    "ld1w { z17.s }, p2/Z, [x15, #4, MUL VL]\n"
    "fmla z28.s, p2/M, z3.s, z5.s\n"
    "fmla z29.s, p2/M, z3.s, z6.s\n"
    "ld1w { z5.s }, p3/Z, [x23, x16, LSL #2]\n"
    "ldr x23, [x17, #0xd0]\n"
    "fmla z30.s, p2/M, z3.s, z9.s\n"
    "fmla z31.s, p2/M, z3.s, z13.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #-8, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z6.s\n"
    "fmla z29.s, p2/M, z4.s, z10.s\n"
    "ld1w { z6.s }, p3/Z, [x26, x16, LSL #2]\n"
    "ldr x27, [x17, #0xd8]\n"
    "fmla z30.s, p2/M, z4.s, z13.s\n"
    "fmla z31.s, p2/M, z4.s, z8.s\n"
    "ld1w { z10.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ldr x22, [x17, #0xe0]\n"
    "ld1w { z4.s }, p2/Z, [x15, #-7, MUL VL]\n"
    "fmla z28.s, p2/M, z0.s, z14.s\n"
    "fmla z29.s, p2/M, z0.s, z11.s\n"
    "ld1w { z14.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ldr x26, [x17, #0xf8]\n"
    "fmla z30.s, p2/M, z0.s, z5.s\n"
    "fmla z31.s, p2/M, z0.s, z6.s\n"
    "ld1w { z0.s }, p2/Z, [x15, #-6, MUL VL]\n"
    "fmla z28.s, p2/M, z1.s, z11.s\n"
    "fmla z29.s, p2/M, z1.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x25, x16, LSL #2]\n"
    "ldr x25, [x17, #0xe8]\n"
    "fmla z30.s, p2/M, z1.s, z6.s\n"
    "fmla z31.s, p2/M, z1.s, z10.s\n"
    "ld1w { z1.s }, p2/Z, [x15, #-5, MUL VL]\n"
    "fmla z28.s, p2/M, z2.s, z12.s\n"
    "fmla z29.s, p2/M, z2.s, z9.s\n"
    "ld1w { z12.s }, p3/Z, [x24, x16, LSL #2]\n"
    "ldr x24, [x17, #0xf0]\n"
    "fmla z30.s, p2/M, z2.s, z10.s\n"
    "fmla z31.s, p2/M, z2.s, z11.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #-4, MUL VL]\n"
    "fmla z28.s, p2/M, z3.s, z9.s\n"
    "fmla z29.s, p2/M, z3.s, z13.s\n"
    "ld1w { z9.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ldr x21, [x17, #0x100]\n"
    "fmla z30.s, p2/M, z3.s, z11.s\n"
    "fmla z31.s, p2/M, z3.s, z12.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #-3, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z13.s\n"
    "fmla z29.s, p2/M, z4.s, z8.s\n"
    "ld1w { z13.s }, p3/Z, [x28, x16, LSL #2]\n"
    "ldr x20, [x17, #0x108]\n"
    "fmla z30.s, p2/M, z4.s, z12.s\n"
    "fmla z31.s, p2/M, z4.s, z14.s\n"
    "ld1w { z8.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ld1w { z4.s }, p2/Z, [x15, #-2, MUL VL]\n"
    "fmla z28.s, p2/M, z0.s, z5.s\n"
    "fmla z29.s, p2/M, z0.s, z6.s\n"
    "ld1w { z5.s }, p3/Z, [x23, x16, LSL #2]\n"
    "ldr x23, [x17, #0x110]\n"
    "fmla z30.s, p2/M, z0.s, z9.s\n"
    "fmla z31.s, p2/M, z0.s, z13.s\n"
    "ld1w { z0.s }, p2/Z, [x15, #-1, MUL VL]\n"
    "fmla z28.s, p2/M, z1.s, z6.s\n"
    "fmla z29.s, p2/M, z1.s, z10.s\n"
    "ld1w { z6.s }, p3/Z, [x27, x16, LSL #2]\n"
    "ldr x22, [x17, #0x118]\n"
    "fmla z30.s, p2/M, z1.s, z13.s\n"
    "fmla z31.s, p2/M, z1.s, z5.s\n"
    "ld1w { z1.s }, p2/Z, [x15]\n"
    "fmla z28.s, p2/M, z2.s, z10.s\n"
    "fmla z29.s, p2/M, z2.s, z11.s\n"
    "ld1w { z10.s }, p3/Z, [x25, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z2.s, z5.s\n"
    "fmla z31.s, p2/M, z2.s, z6.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #1, MUL VL]\n"
    "fmla z28.s, p2/M, z3.s, z11.s\n"
    "fmla z29.s, p2/M, z3.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x24, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z3.s, z6.s\n"
    "fmla z31.s, p2/M, z3.s, z8.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #2, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z12.s\n"
    "fmla z29.s, p2/M, z4.s, z14.s\n"
    "ld1w { z12.s }, p3/Z, [x26, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z4.s, z8.s\n"
    "fmla z31.s, p2/M, z4.s, z10.s\n"
    "ld1w { z4.s }, p2/Z, [x15, #3, MUL VL]\n"
    "addvl x15, x15, #5\n"
    "fmla z28.s, p2/M, z0.s, z9.s\n"
    "fmla z29.s, p2/M, z0.s, z13.s\n"
    "ld1w { z9.s }, p3/Z, [x21, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z0.s, z11.s\n"
    "fmla z31.s, p2/M, z0.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ldp x21, x20, [x17, #0]\n"
    "fmla z28.s, p2/M, z1.s, z13.s\n"
    "fmla z29.s, p2/M, z1.s, z5.s\n"
    "fmla z30.s, p2/M, z1.s, z12.s\n"
    "fmla z31.s, p2/M, z1.s, z9.s\n"
    "ld1w { z12.s }, p3/Z, [x23, x16, LSL #2]\n"
    "fmla z28.s, p2/M, z2.s, z5.s\n"
    "fmla z29.s, p2/M, z2.s, z6.s\n"
    "ld1w { z5.s }, p1/Z, [x21, x14, LSL #2]\n"
    "fmla z30.s, p2/M, z2.s, z9.s\n"
    "fmla z31.s, p2/M, z2.s, z11.s\n"
    "ld1w { z9.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ldp x27, x26, [x17, #0x10]\n"
    "ldp x25, x24, [x17, #0x20]\n"
    "incw x16\n"
    "ldp x23, x22, [x17, #0x30]\n"
    "whilelt p3.s, x16, %x[n_channels]\n"
    "fmla z28.s, p2/M, z3.s, z6.s\n"
    "fmla z29.s, p2/M, z3.s, z8.s\n"
    "ld1w { z6.s }, p1/Z, [x20, x14, LSL #2]\n"
    "ldp x21, x20, [x17, #0x40]\n"
    "fmla z30.s, p2/M, z3.s, z11.s\n"
    "fmla z31.s, p2/M, z3.s, z12.s\n"
    "ld1w { z7.s }, p1/Z, [x27, x14, LSL #2]\n"
    "ld1w { z13.s }, p1/Z, [x24, x14, LSL #2]\n"
    "ld1w { z11.s }, p1/Z, [x23, x14, LSL #2]\n"
    "fmla z28.s, p2/M, z4.s, z8.s\n"
    "fmla z29.s, p2/M, z4.s, z10.s\n"
    "ld1w { z8.s }, p1/Z, [x26, x14, LSL #2]\n"
    "fmla z30.s, p2/M, z4.s, z12.s\n"
    "fmla z31.s, p2/M, z4.s, z9.s\n"
    "ld1w { z9.s }, p1/Z, [x25, x14, LSL #2]\n"
    "ld1w { z12.s }, p1/Z, [x22, x14, LSL #2]\n"
    "ld1w { z10.s }, p1/Z, [x21, x14, LSL #2]\n"
    "ld1w { z14.s }, p1/Z, [x20, x14, LSL #2]\n"
    "incw x14\n"
    ".inst 0xa040c1e0  // ld1w { z0.s-z3.s }, pn8.b/Z, [x15]\n"
    "addvl x15, x15, #4\n"
    "cmp x14, %x[n_channels]\n"
    ".inst 0xc1b0ca5c  // fclamp { z28.s-z31.s }, z18.s, z16.s\n"
    "ld1w { z4.s }, p2/Z, [x15]\n"
    "addvl x15, x15, #1\n"
    "st1w { z28.s }, p0, [x13, x11, LSL #2]\n"
    "st1w { z29.s }, p0, [x12, x11, LSL #2]\n"
    "st1w { z30.s }, p0, [x10, x11, LSL #2]\n"
    "st1w { z31.s }, p0, [x9, x11, LSL #2]\n"
    "blt 1b\n"
    "2:"  // Channel tail
    "movprfx z28, z17\n fmla z28.s, p2/M, z0.s, z5.s\n"
    "movprfx z29, z17\n fmla z29.s, p2/M, z0.s, z6.s\n"
    "ldr x22, [x17, #0x50]\n"
    "incw x11\n"
    "movprfx z30, z17\n fmla z30.s, p2/M, z0.s, z7.s\n"
    "movprfx z31, z17\n fmla z31.s, p2/M, z0.s, z8.s\n"
    "ldr x21, [x17, #0x58]\n"
    "ld1w { z0.s }, p2/Z, [x15]\n"
    "ldr x20, [x17, #0x60]\n"
    "mov p0.b, p3.b\n"
    "ld1w { z5.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ldr x25, [x17, #0x68]\n"
    "fmla z28.s, p2/M, z1.s, z6.s\n"
    "fmla z29.s, p2/M, z1.s, z9.s\n"
    "ld1w { z6.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ldr x24, [x17, #0x70]\n"
    "fmla z30.s, p2/M, z1.s, z8.s\n"
    "fmla z31.s, p2/M, z1.s, z13.s\n"
    "ld1w { z1.s }, p2/Z, [x15, #1, MUL VL]\n"
    "ldr x23, [x17, #0x78]\n"
    "ldr x27, [x17, #0x80]\n"
    "ldr x22, [x17, #0x88]\n"
    "fmla z28.s, p2/M, z2.s, z9.s\n"
    "fmla z29.s, p2/M, z2.s, z11.s\n"
    "ld1w { z9.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ldr x21, [x17, #0x90]\n"
    "fmla z30.s, p2/M, z2.s, z13.s\n"
    "fmla z31.s, p2/M, z2.s, z5.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #2, MUL VL]\n"
    "ldr x26, [x17, #0x98]\n"
    "ldr x20, [x17, #0xa0]\n"
    "fmla z28.s, p2/M, z3.s, z11.s\n"
    "fmla z29.s, p2/M, z3.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x25, x16, LSL #2]\n"
    "ldr x25, [x17, #0xa8]\n"
    "fmla z30.s, p2/M, z3.s, z5.s\n"
    "fmla z31.s, p2/M, z3.s, z6.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #3, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z12.s\n"
    "fmla z29.s, p2/M, z4.s, z9.s\n"
    "ld1w { z12.s }, p3/Z, [x24, x16, LSL #2]\n"
    "ldr x24, [x17, #0xb0]\n"
    "fmla z30.s, p2/M, z4.s, z6.s\n"
    "fmla z31.s, p2/M, z4.s, z10.s\n"
    "ld1w { z9.s }, p3/Z, [x23, x16, LSL #2]\n"
    "ldr x23, [x17, #0xb8]\n"
    "ld1w { z4.s }, p2/Z, [x15, #4, MUL VL]\n"
    "fmla z28.s, p2/M, z0.s, z7.s\n"
    "fmla z29.s, p2/M, z0.s, z8.s\n"
    "fmla z30.s, p2/M, z0.s, z14.s\n"
    "fmla z31.s, p2/M, z0.s, z11.s\n"
    "ld1w { z0.s }, p2/Z, [x15, #5, MUL VL]\n"
    "fmla z28.s, p2/M, z1.s, z8.s\n"
    "fmla z29.s, p2/M, z1.s, z13.s\n"
    "ld1w { z8.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ldr x28, [x17, #0xc8]\n"
    "fmla z30.s, p2/M, z1.s, z11.s\n"
    "fmla z31.s, p2/M, z1.s, z12.s\n"
    "ld1w { z1.s }, p2/Z, [x15, #6, MUL VL]\n"
    "fmla z28.s, p2/M, z2.s, z13.s\n"
    "fmla z29.s, p2/M, z2.s, z5.s\n"
    "ld1w { z13.s }, p3/Z, [x27, x16, LSL #2]\n"
    "ldr x22, [x17, #0xc0]\n"
    "fmla z30.s, p2/M, z2.s, z12.s\n"
    "fmla z31.s, p2/M, z2.s, z9.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #7, MUL VL]\n"
    "addvl x15, x15, #16\n"
    "fmla z28.s, p2/M, z3.s, z5.s\n"
    "fmla z29.s, p2/M, z3.s, z6.s\n"
    "ld1w { z5.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ldr x21, [x17, #0xd0]\n"
    "fmla z30.s, p2/M, z3.s, z9.s\n"
    "fmla z31.s, p2/M, z3.s, z13.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #-8, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z6.s\n"
    "fmla z29.s, p2/M, z4.s, z10.s\n"
    "ld1w { z6.s }, p3/Z, [x26, x16, LSL #2]\n"
    "ldr x27, [x17, #0xd8]\n"
    "fmla z30.s, p2/M, z4.s, z13.s\n"
    "fmla z31.s, p2/M, z4.s, z8.s\n"
    "ld1w { z10.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ldr x20, [x17, #0xe0]\n"
    "ld1w { z4.s }, p2/Z, [x15, #-7, MUL VL]\n"
    "fmla z28.s, p2/M, z0.s, z14.s\n"
    "fmla z29.s, p2/M, z0.s, z11.s\n"
    "ld1w { z14.s }, p3/Z, [x23, x16, LSL #2]\n"
    "ldr x26, [x17, #0xf8]\n"
    "fmla z30.s, p2/M, z0.s, z5.s\n"
    "fmla z31.s, p2/M, z0.s, z6.s\n"
    "ld1w { z0.s }, p2/Z, [x15, #-6, MUL VL]\n"
    "fmla z28.s, p2/M, z1.s, z11.s\n"
    "fmla z29.s, p2/M, z1.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x25, x16, LSL #2]\n"
    "ldr x25, [x17, #0xe8]\n"
    "fmla z30.s, p2/M, z1.s, z6.s\n"
    "fmla z31.s, p2/M, z1.s, z10.s\n"
    "ld1w { z1.s }, p2/Z, [x15, #-5, MUL VL]\n"
    "fmla z28.s, p2/M, z2.s, z12.s\n"
    "fmla z29.s, p2/M, z2.s, z9.s\n"
    "ld1w { z12.s }, p3/Z, [x24, x16, LSL #2]\n"
    "ldr x24, [x17, #0xf0]\n"
    "fmla z30.s, p2/M, z2.s, z10.s\n"
    "fmla z31.s, p2/M, z2.s, z11.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #-4, MUL VL]\n"
    "fmla z28.s, p2/M, z3.s, z9.s\n"
    "fmla z29.s, p2/M, z3.s, z13.s\n"
    "ld1w { z9.s }, p3/Z, [x22, x16, LSL #2]\n"
    "ldr x23, [x17, #0x100]\n"
    "fmla z30.s, p2/M, z3.s, z11.s\n"
    "fmla z31.s, p2/M, z3.s, z12.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #-3, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z13.s\n"
    "fmla z29.s, p2/M, z4.s, z8.s\n"
    "ld1w { z13.s }, p3/Z, [x28, x16, LSL #2]\n"
    "ldr x22, [x17, #0x108]\n"
    "fmla z30.s, p2/M, z4.s, z12.s\n"
    "fmla z31.s, p2/M, z4.s, z14.s\n"
    "ld1w { z8.s }, p3/Z, [x20, x16, LSL #2]\n"
    "ld1w { z4.s }, p2/Z, [x15, #-2, MUL VL]\n"
    "fmla z28.s, p2/M, z0.s, z5.s\n"
    "fmla z29.s, p2/M, z0.s, z6.s\n"
    "ld1w { z5.s }, p3/Z, [x21, x16, LSL #2]\n"
    "ldr x21, [x17, #0x110]\n"
    "fmla z30.s, p2/M, z0.s, z9.s\n"
    "fmla z31.s, p2/M, z0.s, z13.s\n"
    "ld1w { z0.s }, p2/Z, [x15, #-1, MUL VL]\n"
    "fmla z28.s, p2/M, z1.s, z6.s\n"
    "fmla z29.s, p2/M, z1.s, z10.s\n"
    "ld1w { z6.s }, p3/Z, [x27, x16, LSL #2]\n"
    "ldr x20, [x17, #0x118]\n"
    "fmla z30.s, p2/M, z1.s, z13.s\n"
    "fmla z31.s, p2/M, z1.s, z5.s\n"
    "ld1w { z1.s }, p2/Z, [x15]\n"
    "fmla z28.s, p2/M, z2.s, z10.s\n"
    "fmla z29.s, p2/M, z2.s, z11.s\n"
    "ld1w { z10.s }, p3/Z, [x25, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z2.s, z5.s\n"
    "fmla z31.s, p2/M, z2.s, z6.s\n"
    "ld1w { z2.s }, p2/Z, [x15, #1, MUL VL]\n"
    "fmla z28.s, p2/M, z3.s, z11.s\n"
    "fmla z29.s, p2/M, z3.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x24, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z3.s, z6.s\n"
    "fmla z31.s, p2/M, z3.s, z8.s\n"
    "ld1w { z3.s }, p2/Z, [x15, #2, MUL VL]\n"
    "fmla z28.s, p2/M, z4.s, z12.s\n"
    "fmla z29.s, p2/M, z4.s, z14.s\n"
    "ld1w { z12.s }, p3/Z, [x26, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z4.s, z8.s\n"
    "fmla z31.s, p2/M, z4.s, z10.s\n"
    "ld1w { z4.s }, p2/Z, [x15, #3, MUL VL]\n"
    "fmla z28.s, p2/M, z0.s, z9.s\n"
    "fmla z29.s, p2/M, z0.s, z13.s\n"
    "ld1w { z9.s }, p3/Z, [x23, x16, LSL #2]\n"
    "fmla z30.s, p2/M, z0.s, z11.s\n"
    "fmla z31.s, p2/M, z0.s, z12.s\n"
    "ld1w { z11.s }, p3/Z, [x22, x16, LSL #2]\n"
    "fmla z28.s, p2/M, z1.s, z13.s\n"
    "fmla z29.s, p2/M, z1.s, z5.s\n"
    "fmla z30.s, p2/M, z1.s, z12.s\n"
    "fmla z31.s, p2/M, z1.s, z9.s\n"
    "ld1w { z12.s }, p3/Z, [x21, x16, LSL #2]\n"
    "fmla z28.s, p2/M, z2.s, z5.s\n"
    "fmla z29.s, p2/M, z2.s, z6.s\n"
    "fmla z30.s, p2/M, z2.s, z9.s\n"
    "fmla z31.s, p2/M, z2.s, z11.s\n"
    "ld1w { z9.s }, p3/Z, [x20, x16, LSL #2]\n"
    "fmla z28.s, p2/M, z3.s, z6.s\n"
    "fmla z29.s, p2/M, z3.s, z8.s\n"
    "fmla z30.s, p2/M, z3.s, z11.s\n"
    "fmla z31.s, p2/M, z3.s, z12.s\n"
    "fmla z28.s, p2/M, z4.s, z8.s\n"
    "fmla z29.s, p2/M, z4.s, z10.s\n"
    "fmla z30.s, p2/M, z4.s, z12.s\n"
    "fmla z31.s, p2/M, z4.s, z9.s\n"
    ".inst 0xc1b0ca5c  // fclamp { z28.s-z31.s }, z18.s, z16.s\n"
    "st1w { z28.s }, p0, [x13, x11, LSL #2]\n"
    "st1w { z29.s }, p0, [x12, x11, LSL #2]\n"
    "st1w { z30.s }, p0, [x10, x11, LSL #2]\n"
    "st1w { z31.s }, p0, [x9, x11, LSL #2]\n"
    ".inst 0xd503467f  // SMSTOP\n"
    :
    : [n_channels] "r" ((unsigned long) n_channels), [offsetof_Args_inptrs] "I" (offsetof(Args, inptrs)), [offsetof_args_max] "I" (offsetof(Args, max)), [offsetof_args_min] "I" (offsetof(Args, min)), [offsetof_args_outptrs] "I" (offsetof(Args, outptrs)), [offsetof_args_params] "I" (offsetof(Args, params)), [params_struct] "r" (&params_struct)
    : "cc", "memory", "p0", "p1", "p2", "p3", "p4", "p5", "p6", "p7", "p8", "p9", "p10", "p11", "p12", "p13", "p14", "p15", "x9", "x10", "x11", "x12", "x13", "x14", "x15", "x16", "x17", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27", "x28", "z0", "z1", "z2", "z3", "z4", "z5", "z6", "z7", "z8", "z9", "z10", "z11", "z12", "z13", "z14", "z15", "z16", "z17", "z18", "z19", "z20", "z21", "z22", "z23", "z24", "z25", "z26", "z27", "z28", "z29", "z30", "z31"
  );
}

}  // namespace depthwise
}  // namespace ops
}  // namespace kai

#endif  // defined(__aarch64__)

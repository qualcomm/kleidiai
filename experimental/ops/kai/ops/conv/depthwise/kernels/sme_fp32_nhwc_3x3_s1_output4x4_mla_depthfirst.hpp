...
void sme_fp32_nhwc_3x3_s1_output4x4_mla_depthfirst_indirect_impl(const float *const *const input_ptrs, float *const *const outptrs, const void *params, unsigned int n_channels, const float activation_min, const float activation_max);
void sme_fp32_nhwc_3x3_s1_output4x4_mla_depthfirst_direct_impl(const unsigned int n_tile_rows, const unsigned int n_tile_cols, const float *inptr, int64_t ld_input_row, int64_t ld_input_col, float *outptr, int64_t ld_output_row, int64_t ld_output_col, const void *params, unsigned int n_channels, const float activation_min, const float activation_max);

class sme_fp32_nhwc_3x3_s1_output4x4_mla_depthfirst : public DepthwiseDepthfirstStrategy<float, float, float, float>
{
  private:
  using Parent = DepthwiseDepthfirstStrategy<float, float, float, float>;
  Parent::IndirectKernelType m_indirect_kernel = sme_fp32_nhwc_3x3_s1_output4x4_mla_depthfirst_indirect_impl;
  Parent::DirectKernelType m_direct_kernel = sme_fp32_nhwc_3x3_s1_output4x4_mla_depthfirst_direct_impl;
  ...
  sme_fp32_nhwc_3x3_s1_output4x4_mla_depthfirst(const CPUInfo *)

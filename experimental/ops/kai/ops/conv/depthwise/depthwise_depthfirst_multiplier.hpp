...
class DepthfirstMultiplierStrategy<TInput, TWeight, TOutput, int32_t> : public DepthwiseDepthfirstStrategyCommon<TInput, TWeight, TOutput, int32_t, kai::ops::Requantize32>
{
  using Parent = DepthwiseDepthfirstStrategyCommon<TInput, TWeight, TOutput, int32_t, kai::ops::Requantize32>;
  ...
  void pack_parameters(const DepthwiseArgs &args, void *buffer, const void *biases, const kai::ops::Requantize32 &qp, const void *weights, size_t ld_weight_col, size_t ld_weight_row) const override
  {
...
{
  const kai::ops::VLType m_vl_type;
  const unsigned int m_output_rows, m_output_cols;

  public:
  GenericDepthfirstMultiplierKernelStrategy(unsigned int output_rows, unsigned int output_cols, kai::ops::VLType vl_type)
  ...
  kai::ops::VLType get_vl_type(void) const { return m_vl_type; }
...
class GenericDepthfirstMultiplierStrategy : public DepthwiseDepthfirstStrategyCommon<TInput, TWeight, TOutput, TAccum, OutputStage>
{
  ...
  kai::ops::VLType get_vl_type(void) const override { return m_kern->get_vl_type(); }
...
class InputPatchElement
{
  ...
  static size_t sizeof_input_padding(const WorkspaceArgs<IDepthfirstStrategy, OutputStage> &args)
  {
    ...
    auto input_cols = kai::ops::roundup<size_t>(args.strategy->get_input_cols(), 16 / sizeof(T));
  ...
  static size_t sizeof_input_patch(const WorkspaceArgs<IDepthfirstStrategy, OutputStage> &args)
  {
    ...
    {
      // Round-up the number of columns to be a whole number of QUADS
      auto output_cols = kai::ops::roundup<size_t>(args.strategy->get_output_cols(), 16 / sizeof(T));
...
template <typename TInput, typename TWeight, typename TOutput>
struct StrategyType<false, TInput, TWeight, TOutput, int32_t, kai::ops::Requantize32>
{
  ...
  static void execute(
    const DepthwiseArgs &args, const WorkspaceType *ws, const Type *strat,
    const kai::ops::Requantize32 &qp, const unsigned int,
    const void *parameters, const void *
  )
  {
    strat->get_kernel()(
...
template <typename TInput, typename TWeight, typename TOutput>
struct StrategyType<true, TInput, TWeight, TOutput, int32_t, kai::ops::Requantize32>
{
  using Type = GenericDepthfirstMultiplierStrategy<TInput, TWeight, TOutput, int32_t, kai::ops::Requantize32>;

  template <typename WorkspaceType>
  static void execute(
    const DepthwiseArgs &args, const WorkspaceType *ws, const Type *strat,
    const kai::ops::Requantize32 &qp, const unsigned int start_output_channel,
    const void *parameters, const void *
  )
  {

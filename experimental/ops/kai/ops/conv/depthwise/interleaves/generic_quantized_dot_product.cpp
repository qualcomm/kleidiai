...
)
{
  ...
  const unsigned int iter_length = accumulator_depth_vl * kai::ops::utils::get_vector_length<int32_t>(vl_type);
  const unsigned int n_iters = args.input_channels * kai::ops::iceildiv(args.channel_multiplier, iter_length);

  // Compute the cost of storing the weights
  const unsigned int n_dots_per_kernel_row = kai::ops::iceildiv(args.kernel_cols, 4u);
...
)
{
  ...
  const unsigned int iter_length = accumulator_depth_vl * kai::ops::utils::get_vector_length<int32_t>(vl_type);
  const unsigned int n_iters_per_input_channel = kai::ops::iceildiv(args.channel_multiplier, iter_length);
  const unsigned int n_dots_per_kernel_row = kai::ops::iceildiv(args.kernel_cols, 4u);
...
template void pack_parameters(void *, const int32_t *, const int8_t *, size_t, size_t, const DepthwiseArgs &, const kai::ops::Requantize32 &, kai::ops::VLType, unsigned int);
template void pack_parameters(void *, const int32_t *, const uint8_t *, size_t, size_t, const DepthwiseArgs &, const kai::ops::Requantize32 &, kai::ops::VLType, unsigned int);

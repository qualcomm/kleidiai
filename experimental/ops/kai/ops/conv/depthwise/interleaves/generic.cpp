...
PackingArguments::PackingArguments(
  ...
  kai::ops::VLType vl_type, size_t accumulator_element_size, unsigned int accumulator_depth_vl,
...
size_t get_storage_size_generic(const PackingArguments &packing_args, const DepthwiseArgs &args)
{
  ...
  const unsigned int vl =
    ...
    kai::ops::utils::get_vector_length<uint8_t>(packing_args.vl_type) / packing_args.accumulator_element_size;
  const unsigned int n_packs = kai::ops::iceildiv(args.input_channels * args.channel_multiplier, vl);

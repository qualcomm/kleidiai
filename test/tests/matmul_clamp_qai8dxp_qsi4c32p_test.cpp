...
using ParamT = std::tuple<size_t, bool, MatMulShape, size_t, MatrixPortion, RhsPackType, float>;

class QMatMulClampF32Test : public ::testing::TestWithParam<ParamT> {
    struct TestParams {
...
protected:
    ...
    void SetUp() override {
        ...
        if (p.variant && p.variant->ukernel.fn_is_supported && !p.variant->ukernel.fn_is_supported()) {
            GTEST_SKIP() << "Unsupported CPU feature";
        }

        // GEMV vs GEMM constraints
        if (const bool is_gemm = std::get<1>(GetParam()); !is_gemm) {
    ...
    void SetupCommon(
        const std::array<
            UkernelMatmulPackVariant<
                kai_matmul_clamp_f32_qai8dxp_qsi4c32p_ukernel, kai_qai8dxp_pack_functions, kai_qsi4c32p_pack_functions>,
            ArrN>& variants) {
        const size_t variant_index = std::get<0>(GetParam());
        const MatMulShape& shape = std::get<2>(GetParam());
        const size_t bl = std::get<3>(GetParam());
        const MatrixPortion& portion = std::get<4>(GetParam());
        const RhsPackType rhs_dir = std::get<5>(GetParam());
        const float clamp_ratio = std::get<6>(GetParam());
...
template <>
TestData ReferenceGenerator<KeyT, TestData>::generate_reference(const KeyT& key) {
    TestData ref{};

    const MatMulShape shape = std::get<0>(key);
    const size_t bl = std::get<1>(key);
    const size_t mr = std::get<2>(key);
    const size_t kr = std::get<3>(key);
    const size_t sr = std::get<4>(key);
    const size_t rect_start_row = std::get<5>(key);
    const size_t rect_start_col = std::get<6>(key);
    const size_t rect_height = std::get<7>(key);
    const size_t rect_width = std::get<8>(key);
    const RhsPackType rhs_pack_type = std::get<9>(key);
    const int clamp_pct = std::get<10>(key);
    KAI_UNUSED(std::get<11>(key));  // lhs_pack_key (not needed; LHS packer is fixed for this suite)
    ...
    const auto [cmin, cmax] = find_clamp_range<float>(ref_dst_noclamp.data(), ref.M * ref.N, clamp_ratio);
    ref.clamp = {cmin, cmax};
...
[[maybe_unused]] static void PrintTo(const ParamT& param, std::ostream* os) {
...
/// Neon RHS vectorised packer format is s16s0 this is not relevant for sme2 kernels
...
TEST_P(NeonRhsPackF32Test, EndToEndNeonRhsPack) {
    ...
    if (p.rhs_pack_type != RhsPackType::NxK || (kr / sr != 8 && kr / sr != 4)) {
        GTEST_SKIP() << "NEON RHS packers not applicable";
    }
    if (p.rect.height() == 0 || p.rect.width() == 0) {
        GTEST_SKIP() << "Empty dimension of matrix(" << p.rect.width() << "," << p.rect.height() << ")";
    }
...
template <>
BF16TestData ReferenceGenerator<BF16KeyT, BF16TestData>::generate_reference(const BF16KeyT& key) {
    BF16TestData ref{};

    const MatMulShape shape = std::get<0>(key);
    const size_t bl = std::get<1>(key);
    const size_t mr = std::get<2>(key);
    const size_t nr = std::get<3>(key);
    KAI_UNUSED(nr);
    const size_t kr = std::get<4>(key);
    const size_t sr = std::get<5>(key);
    const size_t rect_start_row = std::get<6>(key);
    const size_t rect_start_col = std::get<7>(key);
    const size_t rect_height = std::get<8>(key);
    const size_t rect_width = std::get<9>(key);
    const RhsPackType rhs_pack_type = std::get<10>(key);
...
TEST_P(QMatMulClampBF16Test, EndToEnd) {
    ...
    const BF16KeyT key{matmul_shape,  bl,           mr,           nr, kr, sr, rect.start_row(), rect.start_col(),
                       rect.height(), rect.width(), rhs_pack_type};
    const BF16TestData& data = getV<BF16KeyT, BF16TestData>(key);
...
// clang-format off
static constexpr std::array gemv_shapes{
    MatMulShape{ 1,  16,  64},
    MatMulShape{ 1, 128, 256},
    MatMulShape{ 1, 512, 256},
};

static constexpr std::array gemm_shapes{
    MatMulShape{ 17,  25,  64},   // odd sizes
    MatMulShape{ 64,  64, 128},   // square, aligned K
    MatMulShape{ 96,  96, 192},   // larger square
    MatMulShape{ 64, 128, 256},   // rectangular
};
...
static constexpr std::array<float, 3> clamp_ratios{
    0.01F,
    0.5F,
    1.0F
};
...
// Allow empty instantiations on hosts without supported variants.
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(QMatMulClampF32Test);
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(NeonRhsPackF32Test);
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(QMatMulClampBF16Test);
INSTANTIATE_TEST_SUITE_P(
    MatMulGemm, QMatMulClampF32Test,
    testing::Combine(
        // clang-format off
        testing::Range<size_t>(0, get_f32_gemm_variants().size()),
        testing::Values(true),
        testing::ValuesIn(gemm_shapes),
        testing::Values(32, 64),  // bl
        testing::ValuesIn(gemm_portions),
        testing::Values(RhsPackType::NxK, RhsPackType::KxN),
        testing::ValuesIn(clamp_ratios)),
    // clang-format on
    testing::PrintToStringParamName());

INSTANTIATE_TEST_SUITE_P(
    MatMulGemv, QMatMulClampF32Test,
    testing::Combine(
        // clang-format off
        testing::Range<size_t>(0, get_f32_gemv_variants().size()),
        testing::Values(false), // is_gemm = false (GEMV)
        testing::ValuesIn(gemv_shapes),
        testing::Values(32, 64),  // block size (bl)
...
INSTANTIATE_TEST_SUITE_P(
    MatMulNeonRhsPackGemm, NeonRhsPackF32Test,
    testing::Combine(
        // clang-format off
        testing::Range<size_t>(0, get_f32_neon_gemm_variants_only().size()),
        testing::Values(true), // is_gemm
        testing::ValuesIn(gemm_shapes),
        testing::Values(32, 64),  // bl
        testing::ValuesIn(gemm_portions),
        testing::Values(RhsPackType::NxK),

...
///
/// -# kai_lhs_quant_pack_qai8dxp_f16_neon to dynamically quantize and pack the LHS matrix
/// -# kai_rhs_pack_nxk_qsi4cxp_qs4cxs1s0 to pack the RHS matrix, using the kr/sr values returned by
///    this micro-kernel (this SME-only variant uses a different kr/sr than its sme2_sdot sibling, so the RHS must
///    be packed specifically for this micro-kernel and cannot be shared with the sme2_sdot packed buffer).
...
/// the @ref kai_lhs_quant_pack_qai8dxp_f16_neon micro-kernel
///
/// @return the mr value
...
/// @ref kai_run_matmul_clamp_f16_qai8dxp1x4_qsi4cxp4vlx4_1x4vl_sme2_sdot. It does not use `luti4`/`zt0` or any
/// SME2 ZA-accumulate instruction, so it only requires FEAT_SME (plus SVE2) rather than FEAT_SME2.
///
...
///                            by calling the @ref kai_lhs_quant_pack_qai8dxp_f16_neon micro-kernel which performs

...
struct kai_matmul_uker_args_internal {
    ...
    size_t k;                  // 0x28
    size_t bl;                 // 0x30
    const int32_t* reserved;   // 0x38
    float min;                 // 0x40
    float max;                 // 0x44
};

void kai_kernel_matmul_clamp_f32_qsi8d32p1x4_qai4c32p16vsx4s4s0sf16_1x16vs_qmx_dot(
...
static void run(const struct kai_matmul_uker_config* config, const struct kai_matmul_uker_args* args) {
    ...
    kai_commit_za();
    kai_kernel_matmul_clamp_f32_qsi8d32p1x4_qai4c32p16vsx4s4s0sf16_1x16vs_qmx_dot(&uker_args);
}

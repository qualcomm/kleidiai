...
void kai_kernel_matmul_clamp_qai8_qai8p4vsx4_qsi8cxp4vsx4sf32bi32_2vsx32vs_sme2p1_mop4(
...
static void run(const struct kai_matmul_uker_config* config, const struct kai_matmul_uker_args* args) {
    ...
    if (args->shape.m > main_m && args->shape.n > 0) {
        ...
        const size_t shape_n_32vs = kai_rounddown(leftover_n, 8 * acc_vl);

        if (leftover_m <= acc_vl / 2 && shape_n_32vs > 0) {
            // Construct uker_args.
            struct kai_matmul_uker_args_internal uker_args = {0};

            uker_args.flags = flags;
            uker_args.m = leftover_m;
            uker_args.n = shape_n_32vs;
            uker_args.k = args->shape.k;
            ...
            kai_kernel_matmul_clamp_qai8_qai8p4vsx4_qsi8cxp4vsx4sf32bi32_2vsx32vs_sme2p1_mop4(&uker_args);

            leftover_n -= shape_n_32vs;
            rhs_ptr += shape_n_32vs / acc_vl * args->operand.rhs.stride.n;
            dst_ptr += shape_n_32vs * DST_ELEM_BYTES;
        }

        if (leftover_m > 0 && leftover_n > 0) {

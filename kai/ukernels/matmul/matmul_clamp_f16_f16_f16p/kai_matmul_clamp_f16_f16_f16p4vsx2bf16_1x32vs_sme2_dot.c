...
void kai_kernel_matmul_clamp_f16_f16_f16p4vsx2bf16_1x32vs_sme2_dot(const struct kai_matmul_uker_args_internal* args);
...
static void run(const struct kai_matmul_uker_config* config, const struct kai_matmul_uker_args* args) {
    ...
    if (args->flags & KAI_MATMUL_UKER_FLAGS_ARGS_CLAMP) {
        const float32x4_t min_f32 = vld1q_dup_f32((const float*)args->activation.clamp.min_ptr);
        const float32x4_t max_f32 = vld1q_dup_f32((const float*)args->activation.clamp.max_ptr);

        const uint16x4_t min = vreinterpret_u16_f16(vcvt_f16_f32(min_f32));
        const uint16x4_t max = vreinterpret_u16_f16(vcvt_f16_f32(max_f32));

        vst1_lane_u16(&uker_args.minval, min, 0);
        vst1_lane_u16(&uker_args.maxval, max, 0);

        uker_args.flags |= 2;
    }

    kai_kernel_matmul_clamp_f16_f16_f16p4vsx2bf16_1x32vs_sme2_dot(&uker_args);
}

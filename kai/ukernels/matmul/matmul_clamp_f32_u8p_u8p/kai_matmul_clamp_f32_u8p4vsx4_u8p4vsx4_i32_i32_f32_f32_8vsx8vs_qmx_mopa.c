...
void kai_kernel_matmul_clamp_f32_u8p4vsx4_u8p4vsx4_i32_i32_f32_f32_8vsx8vs_qmx_mopa(
...
static void run(const struct kai_matmul_uker_config* config, const struct kai_matmul_uker_args* run_args) {
    ...
    KAI_ASSUME(run_args->operand.scale.acc_scale_global.ptr != NULL);
    KAI_ASSUME(run_args->operand.bias.scale_bias_n.ptr != NULL);
    ...
    args.dst_bias_n_ptr = run_args->operand.bias.scale_bias_n.ptr;
    args.dst_scale_1_ptr = run_args->operand.scale.acc_scale_global.ptr;
    args.min_ptr = enable_clamp ? run_args->activation.clamp.min_ptr : &float_min;
    args.max_ptr = enable_clamp ? run_args->activation.clamp.max_ptr : &float_max;

    kai_commit_za();
    kai_kernel_matmul_clamp_f32_u8p4vsx4_u8p4vsx4_i32_i32_f32_f32_8vsx8vs_qmx_mopa(&args);
}

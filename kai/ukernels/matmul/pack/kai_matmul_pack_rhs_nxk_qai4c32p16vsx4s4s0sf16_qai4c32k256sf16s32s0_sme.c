...
static inline uint8x8_t pack_s4s0_packets(uint8x16_t nibbles) {
    static const uint8_t low_indices[8] = {0, 1, 2, 3, 8, 9, 10, 11};
    static const uint8_t high_indices[8] = {4, 5, 6, 7, 12, 13, 14, 15};
    const uint8x8_t low = vqtbl1_u8(nibbles, vld1_u8(low_indices));
    const uint8x8_t high = vqtbl1_u8(nibbles, vld1_u8(high_indices));
    return vorr_u8(low, vshl_n_u8(high, 4));
}

static inline uint8x16x2_t convert_s32s0_subblock_pair_to_s4s0(const qai4c32k256* src_block, size_t subblock_pair_idx) {
    ...
    const uint8x16_t sign_xor = vdupq_n_u8(0x88U);

    // Native s32s0 shares each source byte between two subblocks. Repack each subblock into four 8-K packets:
    // byte 4*p+b contains K[8*p+b] in its low nibble and K[8*p+b+4] in its high nibble.
    const uint8x16x2_t result = {
        .val =
            {
                veorq_u8(
                    vcombine_u8(
                        pack_s4s0_packets(vandq_u8(src_0, nibble_mask)),
                        pack_s4s0_packets(vandq_u8(src_1, nibble_mask))),
                    sign_xor),
                veorq_u8(
                    vcombine_u8(pack_s4s0_packets(vshrq_n_u8(src_0, 4)), pack_s4s0_packets(vshrq_n_u8(src_1, 4))),
                    sign_xor),
            },
    };
    return result;
...
static inline void store_interleaved_rows4(
    ...
    const uint32x4_t row_0_u32 = vreinterpretq_u32_u8(row_0);
    const uint32x4_t row_1_u32 = vreinterpretq_u32_u8(row_1);
    const uint32x4_t row_2_u32 = vreinterpretq_u32_u8(row_2);
    const uint32x4_t row_3_u32 = vreinterpretq_u32_u8(row_3);

    const uint32x4_t rows_01_low = vzip1q_u32(row_0_u32, row_1_u32);
    const uint32x4_t rows_23_low = vzip1q_u32(row_2_u32, row_3_u32);
    const uint32x4_t rows_01_high = vzip2q_u32(row_0_u32, row_1_u32);
    const uint32x4_t rows_23_high = vzip2q_u32(row_2_u32, row_3_u32);

    const uint8x16_t packet_0 = vreinterpretq_u8_u64(vzip1q_u64(
        vreinterpretq_u64_u32(rows_01_low), vreinterpretq_u64_u32(rows_23_low)));
    const uint8x16_t packet_1 = vreinterpretq_u8_u64(vzip2q_u64(
        vreinterpretq_u64_u32(rows_01_low), vreinterpretq_u64_u32(rows_23_low)));
    const uint8x16_t packet_2 = vreinterpretq_u8_u64(vzip1q_u64(
        vreinterpretq_u64_u32(rows_01_high), vreinterpretq_u64_u32(rows_23_high)));
    const uint8x16_t packet_3 = vreinterpretq_u8_u64(vzip2q_u64(
        vreinterpretq_u64_u32(rows_01_high), vreinterpretq_u64_u32(rows_23_high)));

    const size_t packet_stride = nr * OUTPUT_BYTES_PER_K_PACKET;
    uint8_t* dst_row_group = dst + row_group * OUTPUT_BYTES_PER_K_PACKET;
    vst1q_u8(dst_row_group, packet_0);
    vst1q_u8(dst_row_group + packet_stride, packet_1);
    vst1q_u8(dst_row_group + 2U * packet_stride, packet_2);
    vst1q_u8(dst_row_group + 3U * packet_stride, packet_3);
}
...
static void pack_quantized_subblock_pair(
    ...
    for (size_t row_group = 0; row_group < nr; row_group += OUTPUT_ROWS_PER_GROUP) {
        ...
        const uint8x16x2_t row_0 = convert_s32s0_subblock_pair_to_s4s0(src_block_0, subblock_pair_idx);
        const uint8x16x2_t row_1 = convert_s32s0_subblock_pair_to_s4s0(src_block_1, subblock_pair_idx);
        const uint8x16x2_t row_2 = convert_s32s0_subblock_pair_to_s4s0(src_block_2, subblock_pair_idx);
        const uint8x16x2_t row_3 = convert_s32s0_subblock_pair_to_s4s0(src_block_3, subblock_pair_idx);
...
static void pack_metadata(
    ...
    for (size_t row_idx = 0; row_idx < nr; ++row_idx) {
        ...
        const float resolved_scale = superblock_scale * (float)scale_offset.scale;
        const float resolved_offset = -superblock_offset * (float)scale_offset.offset;

        // The payload contains signed (q - 8) nibbles in the high half of each SDOT byte operand.
        const float packed_scale = resolved_scale / 16.0F;
        const float packed_offset = resolved_offset + 8.0F * resolved_scale;
        const size_t dst_idx = row_idx < half_nr ? 2U * row_idx : 2U * (row_idx - half_nr) + 1U;
        dst_offsets[dst_idx] = kai_cast_f16_f32(packed_offset);
        dst_scales[dst_idx] = kai_cast_f16_f32(packed_scale);
    }
}

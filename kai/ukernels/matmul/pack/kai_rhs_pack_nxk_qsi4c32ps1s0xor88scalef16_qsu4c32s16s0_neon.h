...
size_t kai_get_rhs_offset_rhs_pack_nxk_qsi4c32ps1s0xor88scalef16_qsu4c32s16s0_neon(
    size_t n_idx,        //
    size_t rhs_stride);  //
...
/// @ref kai_run_rhs_pack_nxk_qsi4c32ps1s0scalef16_qsu4c32s16s0_neon, plus XOR 0x88 on every packed byte
/// (converts the zero-point-8 nibble values to signed two's-complement, so the matching micro-kernel can
/// use a signed widen instead of a runtime zero-point subtraction). The micro-kernel's signed widen shifts
/// each nibble to the top of its byte first, which scales every decoded value by 16 -- this packer
/// pre-divides every packed fp16 scale by 16 to compensate.
///
...
/// @param[in]  extra_bytes Extra bytes to append to the end of each row of the packed RHS matrix. Must be 0.

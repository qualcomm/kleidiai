...
/// Mirrors the vaddlvq_u8 idiom in kai_rhs_pack_kxn_qsi4cxp8vsx4sf32bi32_qsx4cx_f32_i32_sme.c.
...
/// Nibble (n, k) of a KxN i4 matrix: row-major over k, two nibbles per byte along n.
static inline uint8_t kai_rhs_nibble_kxn(const uint8_t* rhs, size_t n_idx, size_t stride, size_t k_idx) {
    const uint8_t byte = rhs[k_idx * stride + n_idx / 2];
    return (n_idx % 2 == 0) ? (uint8_t)(byte & 0x0FU) : (uint8_t)(byte >> 4);
}
...
/// @ref kai_run_rhs_pack_kxn_qsi4cxp8vsx4sf32bi32_qsx4cx_f32_i32_sme, and the bias/scale
...
void kai_run_rhs_pack_kxn_qsi4cxp8vsx4s4s0sf32bi32_qsx4cx_f32_i32_sme(
    const struct kai_matmul_pack_rhs_uker_config* config, const struct kai_matmul_pack_rhs_uker_args* args,
    ...
    for (size_t n_base = 0; n_base < n; n_base += nr) {
        ...
        for (size_t group = 0; group < block_width; ++group) {
            ...
            for (size_t k_base = 0; k_base < k; k_base += K_SPAN) {
                ...
                // KxN stores K down rows, so one column's 16 K values come from 16 different
                // rows -- no contiguous load is available. The gather stays scalar; the pairing and
                // the row sum below are still vectorised.
                uint8_t span_nibbles[K_SPAN];
                for (size_t t = 0; t < K_SPAN; ++t) {
                    const size_t k_idx = k_base + t;
                    span_nibbles[t] = (k_idx < k) ? kai_rhs_nibble_kxn(rhs_ptr, n_idx, rhs_stride_row, k_idx)
                                                  : (uint8_t)rhs_zero_point;
                }
                nibbles = vld1q_u8(span_nibbles);
        ...
        // Bias/scale epilogue, unchanged from kai_run_rhs_pack_kxn_qsi4cxp8vsx4sf32bi32_qsx4cx.
        size_t group = 0;

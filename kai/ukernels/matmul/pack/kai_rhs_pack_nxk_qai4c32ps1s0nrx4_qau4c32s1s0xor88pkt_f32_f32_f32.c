...
#include "kai_rhs_pack_nxk_qai4c32ps1s0nrx4_qau4c32s1s0xor88pkt_f32_f32_f32.h"
...
size_t kai_get_rhs_offset_rhs_pack_nxk_qai4c32ps1s0nrx4_qau4c32s1s0xor88pkt_f32_f32_f32(
    size_t n_idx, size_t rhs_stride) {
    return n_idx * rhs_stride;
}

size_t kai_get_rhs_packed_offset_rhs_pack_nxk_qai4c32ps1s0nrx4_qau4c32s1s0xor88pkt_f32_f32_f32(
...
size_t kai_get_rhs_packed_size_rhs_pack_nxk_qai4c32ps1s0nrx4_qau4c32s1s0xor88pkt_f32_f32_f32(
...
//
// This is the closed form of the per-nibble definition. With the s1s0 source order (source
// column c holds k=2c in its low nibble and k=2c+1 in its high nibble), packet p maps to
// source columns 4p..4p+3, and the four output bytes draw only on (s0,s2) and (s1,s3):
//   out0 = s0.lo | s2.lo<<4      out1 = s0.hi | s2.hi<<4
//   out2 = s1.lo | s3.lo<<4      out3 = s1.hi | s3.hi<<4
// so the whole packet reduces to a handful of bit operations with no per-nibble indexing.
inline static void kai_pack_packet(const uint8_t* src, uint8_t* dst) {
    ...
    dst[0] = (uint8_t)(((s0 & 0x0F) | (uint8_t)((s2 & 0x0F) << 4)) ^ kai_sign_xor_value);
    dst[1] = (uint8_t)(((uint8_t)(s0 >> 4) | (s2 & 0xF0)) ^ kai_sign_xor_value);
    dst[2] = (uint8_t)(((s1 & 0x0F) | (uint8_t)((s3 & 0x0F) << 4)) ^ kai_sign_xor_value);
    dst[3] = (uint8_t)(((uint8_t)(s1 >> 4) | (s3 & 0xF0)) ^ kai_sign_xor_value);
}

void kai_run_rhs_pack_nxk_qai4c32ps1s0nrx4_qau4c32s1s0xor88pkt_f32_f32_f32(
